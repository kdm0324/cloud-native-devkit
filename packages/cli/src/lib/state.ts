...
export const ensureStateDir = () => fs.mkdirSync(STATE_DIR, { recursive: true });
...
export const loadState = () => {
  if (!fs.existsSync(STATE_PATH)) throw new Error(`state 파일이 없습니다. 먼저 'local-dev init'을 실행하세요. (${STATE_PATH})`);
  return JSON.parse(fs.readFileSync(STATE_PATH, "utf-8")) as { namespace: string; release: string };
};
...
export const loadSpec = () => {
  if (!fs.existsSync(SPEC_PATH)) throw new Error(`spec 파일이 없습니다. 먼저 'local-dev init'을 실행하세요. (${SPEC_PATH})`);
